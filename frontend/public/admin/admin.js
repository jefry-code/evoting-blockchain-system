 ...
 * KONFIGURASI GLOBAL
 */
const BACKEND_URL = "https://5521-114-122-14-85.ngrok-free.app";
const NGROK_HEADERS = {
...
const themeHandler = {
  ...
  updateActiveUI: (theme) => {
    ...
    document.querySelectorAll("[data-bs-theme-value]").forEach((el) => {
      el.classList.toggle(
        "active",
        el.getAttribute("data-bs-theme-value") === theme,
      );
    });
  },
};
...
function initEventListeners() {
  ...
  if (logoutBtnSidebar)
    logoutBtnSidebar.onclick = (e) => {
      e.preventDefault();
      confirmLogout();
    };
  if (logoutBtnNavbar)
    logoutBtnNavbar.onclick = (e) => {
      e.preventDefault();
      confirmLogout();
    };
  ...
  document.getElementById("menuKandidat").onclick = (e) => {
    e.preventDefault();
    showKandidatData();
  };
}
  ...
  try {
    ...
    if (isAuth === "true" && window.ethereum) {
      ...
      const currentAddress =
        accounts.length > 0 ? accounts[0].address.toLowerCase() : null;
      ...
      if (
        ...
        currentAddress === AUTHORIZED_ADMIN
      ) {
...
async function connectWallet() {
  ...
  btnConnect.innerHTML =
    '<span class="spinner-border spinner-border-sm me-2"></span>Menghubungkan...';
  ...
  try {
    ...
    if (typeof window.ethereum === "undefined") {
      throw new Error(
        "Setelah instalasi selesai, silakan refresh halaman ini.",
      );
    }
    ...
    try {
      ...
      if (signErr.code === "ACTION_REJECTED" || signErr.code === 4001) {
        throw new Error(
          "Login dibatalkan: Tanda tangan diperlukan untuk akses ke dashboard.",
        );
      }
      throw signErr;
    }
    ...
    addLog(
      `Admin login berhasil: ${adminAddress.substring(
        0,
        6,
      )}...${adminAddress.substring(adminAddress.length - 4)}`,
      "success",
    );
  } catch (err) {
    ...
    if (err.code === "ACTION_REJECTED" || err.code === 4001) {
      status.innerText =
        "Akun belum tersedia! Silahkan masuk menggunakan akun admin";
    } else {
...
function showDashboard(address) {
  ...
  if (window.ethereum) {
    ...
    window.ethereum.on("accountsChanged", (accounts) => {
      if (
        accounts.length === 0 ||
        accounts[0].toLowerCase() !== AUTHORIZED_ADMIN
      ) {
        showAdminAuthModal();
      }
    });
  ...
  const displayAddr = `${address.substring(0, 6)}...${address.substring(
    address.length - 4,
  )}`;
...
function updateDashboardUI(candidates) {
  ...
  const participation =
    TOTAL_DPT > 0 ? ((totalVotes / TOTAL_DPT) * 100).toFixed(1) : 0;
  ...
  if (elTotalVoters)
    elTotalVoters.innerText = TOTAL_DPT.toLocaleString("id-ID");
                ...
                <td>
                    <div class="d-flex align-items-center">
                        <img src="${getFullImageUrl(
                          cand.foto,
                        )}" class="rounded-circle me-3 border border-secondary" width="40" height="40" style="object-fit: cover;" onerror="this.src='/img/default.png'">
                        <div>
                            <div class="fw-bold">${cand.nama}</div>
                            <div class="small text-muted">Kandidat No. ${
                              cand.noUrut
                            }</div>
                        </div>
                    </div>
...
function renderTransactionTableRows() {
  ...
  if (votedVotersOnly.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="text-center py-5 text-muted">Menunggu transaksi pertama masuk...</td></tr>';
    if (txNav) txNav.classList.add("d-none");
    return;
  }

  if (syncText)
    syncText.innerText = `Last Update: ${new Date().toLocaleTimeString(
      "id-ID",
    )}`;
  ...
  tbody.innerHTML = paginatedTx
    .map((voter, index) => {
      const txHash = voter.txHash || "";
      const shortTx = `${txHash.substring(0, 10)}...${txHash.substring(60)}`;
      const shortNik = `${voter.nikHash.substring(
        0,
        10,
      )}...${voter.nikHash.substring(54)}`;
      ...
      return `
            <tr>
                <td class="ps-4 text-muted mono" style="font-size: 0.75rem;">${
                  start + index + 1
                }</td>
                <td>
                ...
                <td class="small text-muted live-time" data-time="${
                  voter.timestamp
                }">
  ...
  // Logic Navigasi
  if (txNav) {
    votedVotersOnly.length > limit
      ? txNav.classList.remove("d-none")
      : txNav.classList.add("d-none");
...
async function startCountdownTimer() {
  try {
    ...
    if (data.status === "active" && data.targetTime) {
      ...
      document.getElementById("statTimer").innerText =
...
async function showVoterData() {
  ...
  try {
    ...
    document.getElementById("voterTableBody").innerHTML =
      '<tr><td colspan="3">Gagal load data.</td></tr>';
  }
}
...
function renderVoterTable() {
  ...
  if (paginatedItems.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="3" class="text-center py-4 text-muted">Data tidak ditemukan.</td></tr>';
  ...
  document.getElementById("paginationInfo").innerText = `Menampilkan ${
    start + 1
  } - ${Math.min(end, filteredVoters.length)} dari ${filteredVoters.length}`;
...
async function showKandidatData() {
  ...
  container.innerHTML =
    '<div class="text-center p-5 w-100"><div class="spinner-border text-primary"></div></div>';
  modal.show();
                ...
                <div class="card h-100 card-custom border-0 shadow-lg">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="p-3 rounded-3 mb-3" style="background: var(--input-bg); border: 1px solid var(--border);">
                            ...
                            <p class="small mb-0 text-truncate-3" style="color: var(--text-main);">${
                              k.visi
                            }</p>
                        </div>
...
function showCandidateDetail(index) {
  ...
  document.getElementById(
    "modalHeaderColor",
  ).style.background = `linear-gradient(135deg, ${cand.warna} 0%, ${cand.warna}dd 100%)`;
...
async function executeVotingActivation(modalBtn) {
  ...
  try {
    ...
    if (modalBtn)
      modalBtn.innerHTML =
        '<span class="spinner-border spinner-border-sm me-2"></span>Konfirmasi di MetaMask...';
    ...
    if (modalBtn)
      modalBtn.innerHTML =
        '<span class="spinner-border spinner-border-sm me-2"></span>Menunggu Konfirmasi...';
    ...
    if (receipt.status === 1) {
      ...
      addLog(
        `Sistem: Durasi berlangsung ${DEFAULT_VOTING_DURATION} Jam.`,
        "info",
      );
...
async function refreshDashboardStatus() {
  try {
    ...
    if (data.status.toLowerCase() === "active") {
      ...
      btnStart.innerHTML =
        '<i class="bi bi-check-all me-2"></i>Voting Berlangsung';
      ...
      btnStart.disabled = true;
      btnStart.innerHTML =
        '<i class="bi bi-slash-circle me-2"></i>Voting Telah Berakhir';
