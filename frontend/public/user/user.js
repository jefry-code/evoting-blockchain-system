...
async function loadCandidates() {
  try {
    ...
    grid.innerHTML = data
      .map(
        (cand, index) => `
            <div class="col-md-6 col-lg-4" style="animation: fadeIn 0.6s ease forwards ${
              index * 0.1
            }s; opacity: 0;">
                <div class="candidate-card" onclick="openVoteModal('${
                  cand.id
                }', '${cand.nama}', '${cand.noUrut}', '${cand.foto}')">
          ...
          cand.nama
                    ...
                    <h4 class="fw-extrabold mb-1 tracking-tight">${
                      cand.nama
                    }</h4>
...
function openVoteModal(id, nama, noUrut, foto) {
  ...
  document.getElementById(
    "confirmNoUrut",
  ).innerText = `Kandidat Nomor ${noUrut}`;
...
function initSwipeLogic() {
  ...
  handle.addEventListener("pointermove", (e) => {
    ...
    document.querySelector(".swipe-text").style.opacity = Math.max(
      opacityValue,
      0.1,
    );
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.isDoubleVote) {
        ...
        showErrorVoteModal(
          data.error || "Pilihan Anda ditolak oleh sistem.",
          false,
          null,
        );
      }
...
function showConcurrentAlert(customMessage) {
  ...
  const concurrentModal = new bootstrap.Modal(
    document.getElementById("concurrentModal"),
  );
...
function showErrorVoteModal(message, isDoubleVote, txHash) {
  ...
  const errorModal = new bootstrap.Modal(
    document.getElementById("errorVoteModal"),
  );
  errorModal.show();
...
function showNetworkErrorModal() {
  ...
  const netModal = new bootstrap.Modal(
    document.getElementById("networkErrorModal"),
  );
  netModal.show();
}
  ...
  try {
    ...
    if (data.status === "active") {
      ...
      if (swipeTrack) {
        ...
        document.querySelector(".swipe-text").innerText =
...
function runTimer(targetTime, displayElement, onFinish) {
  function update() {
    ...
    displayElement.innerText = `${String(h).padStart(2, "0")}:${String(
      m,
